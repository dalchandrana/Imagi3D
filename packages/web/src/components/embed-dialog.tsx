 ...
 * =============================================================================
 *
 * Dialog that generates copyable embed code for the current editor state.
 * Styled as numbered installation steps with syntax-highlighted code blocks.
 */
...
function generateProps(props: EmbedDialogProps): string {
  ...
  if (props.smoothness !== 0.2) lines.push(`  smoothness={${props.smoothness}}`);
  ...
  if (props.textureUrl) {
    ...
    if (props.textureSettings.offsetX !== defaultTextureSettings.offsetX || props.textureSettings.offsetY !== defaultTextureSettings.offsetY) {
      lines.push(`  textureOffset={[${props.textureSettings.offsetX}, ${props.textureSettings.offsetY}]}`);
    }
  }

  if (props.animate !== "none") {
    lines.push(`  animate="${props.animate}"`);
    if (props.animateSpeed !== 1) lines.push(`  animateSpeed={${props.animateSpeed}}`);
    if (props.animateReverse) lines.push(`  animateReverse`);
  }

  if (props.rotationX !== 0) lines.push(`  rotationX={${round(props.rotationX)}}`);
  if (props.rotationY !== 0) lines.push(`  rotationY={${round(props.rotationY)}}`);
  if (props.zoom !== 8) lines.push(`  zoom={${props.zoom}}`);

  if (props.cursorOrbit) {
    lines.push(`  cursorOrbit`);
    if (props.orbitStrength !== 0.15) lines.push(`  orbitStrength={${props.orbitStrength}}`);
  }
  if (props.resetOnIdle) {
    lines.push(`  resetOnIdle`);
    if (props.resetDelay !== 2) lines.push(`  resetDelay={${props.resetDelay}}`);
  }
  ...
  if (ls.keyIntensity !== dl.keyIntensity) lines.push(`  lightIntensity={${ls.keyIntensity}}`);
  if (ls.ambientIntensity !== dl.ambientIntensity) lines.push(`  ambientIntensity={${ls.ambientIntensity}}`);
...
function formatCode(code: string) {
  return code.split("\n").map((line, i) => (
    <span key={i} className="block">
      <span className="inline-block w-7 text-right mr-4 text-white/20 select-none">{i + 1}</span>
      {line}
    </span>
  ));
}
  ...
  return (
    <div className="pb-4 min-w-0">
      <div className="flex items-center gap-2.5 mb-2">
        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-[10px] font-medium text-foreground/70">{number}</span>
...
function CodeBlock({ code, label, onCopy, copied, lineNumbers = true }: { code: string; label: string; onCopy: () => void; copied: boolean; lineNumbers?: boolean }) {
  return (
    <div className="rounded-lg border border-white/[0.1] bg-black/70 overflow-hidden mt-2 min-w-0">
      {label && (
...
export function EmbedDialog(props: EmbedDialogProps) {
  ...
  const embedCode = useMemo(() => {
    ...
    if (!propsStr) return `<SVG3D />`;
    return `<SVG3D\n${propsStr}\n/>`;
  }, [props]);

  const fullSnippet = useMemo(() => {
    const importLine = `import { SVG3D } from "imagi3d";`;
    if (!isTextMode) {
      ...
      <DialogContent className="sm:max-w-lg bg-card/70 backdrop-blur-xl border-white/[0.06] shadow-[0_8px_32px_oklch(0_0_0/0.4)]">
        ...
        <div className="mt-1">
          <Step number={1} title="Install the package">
            <CodeBlock
              code="npm install imagi3d"
              label="Terminal"
              onCopy={() => handleCopy("npm install imagi3d", "npm")}
              copied={copied === "npm"}
              lineNumbers={false}
            />
          </Step>

          <Step number={2} title={isTextMode ? "Add the component to your project" : "Copy and paste into your component"}>
            <CodeBlock
              code={fullSnippet}
